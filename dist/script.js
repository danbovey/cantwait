...
[].forEach.call(widgets, function (el) {
  ...
  var trackArtworkLink = el.querySelector('.track-artwork-link');
  ...
  var displayTrack = function displayTrack(track) {
    ...
    trackArtworkLink.setAttribute('href', permalink); // Load the waveform JSON file
  ...
  var waveformOffset = trackWaveform.getBoundingClientRect().left;
  var waveformWidth = trackWaveform.clientWidth;
  trackWaveform.addEventListener('click', function (e) {
    // Reset waveform fill
    var bars = trackWaveform.querySelectorAll('.bar');
    bars.forEach(function (bar) {
      bar.style.background = '#FFF';
    });
    var percent = (e.clientX - waveformOffset) / waveformWidth;
    widget.getDuration(function (duration) {
      widget.seekTo(duration * percent); // In case we aren't playing yet
